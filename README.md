...
| `application-version` | Желаемая версия приложения для деплоя. Если версия не заполнена, то берется из pom.xml. Переменной окружения VERSION будет присвоено значение версии. | `''` | `string` |
...
3. **Настройка переменных окружения**
    ...
    - Добавляется переменная окружения `NAMESPACE` из `inputs.kube-namespace` и сохраняется в `GITHUB_ENV`.
